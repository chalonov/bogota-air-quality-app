...
FEATURES = ['dayofyear', 'hour', 'dayofweek', station_pm10]
TARGET = station_pm25
...
#             columns=['importance'])
#fi.sort_values('importance').plot(kind='barh', title='Feature Importance', figsize=(14, 3))
#plt.grid()
#plt.show()
...
#ax = df.loc[(df.index > '06-01-2023 ') & (df.index < '06-15-2023')][station_pm25].plot(figsize=(15, 5), title='Últimos 15 dias')
#df.loc[(df.index > '06-01-2023') & (df.index < '06-15-2023')]['prediction'].plot(style='--')
#plt.legend(['Truth Data','Prediction'])
#plt.grid()
#plt.show()

score = np.sqrt(mean_squared_error(test[station_pm25], test['prediction']))
...
with open('pickle/model_' + station_pm25, 'wb') as handle:
    pickle.dump(df, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('pickle/score_' + station_pm25, 'wb') as handle:
